(function () {
  ...
  function updateRunState() {
    runBtn.disabled = !(primaryFile && secondaryFile);
  }

  function resetOutputTiles() {
    [wordTile, pdfTile].forEach(function (el) {
  ...
  function applyOutputHref(tile, href) {
    ...
    if (hint) {
      hint.textContent = "Open / download";
    ...
    fetch("/compare", { method: "POST", body: form })
      ...
      .then(function (result) {
        ...
        if (extras && typeof extras === "object") {
          ...
          applyOutputHref(pdfTile, extras.output_pdf_href);
        }
      })
      .catch(function () {
        pathLoading.hidden = true;
